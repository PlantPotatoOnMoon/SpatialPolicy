    ...
    try:
        ...
        for line in lines:
            ...
            if len(parts) >= 2:
                ...
                action_to_id = {'forward': 0, 'left': 1, 'right': 2, 'done': 3, 'pad': 4}
                action_id = action_to_id.get(action, 4)  
...
class SequentialGifDataset(Dataset):
        ...
        for idx, gif_path in enumerate(tqdm(gif_files)):
            ...
            try:
                ...
                if len(gif_frames) >= 2:  
                    ...
                    print(f"Warning: {gif_path} only has {len(gif_frames)} frame")
            except Exception as e:
                print(f"Warning: Can not read {gif_path}: {e}")
    ...
    def __getitem__(self, idx):
        ...
        images_tensor = torch.stack(images)  # [8, C, H, W]
        
        x_cond = images_tensor[0]  # [C, H, W]
        
        x = images_tensor[1:].flatten(0, 1)  # [7*C, H, W] = [21, H, W]
