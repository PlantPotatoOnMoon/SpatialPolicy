...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if task_tokens:
            #imagen训练含layout的？
            #PerceiverResampler是根据y生成一个embedding，embedding的维度是task_token_channels
            self.task_attnpool = nn.Sequential(
                PerceiverResampler(dim=task_token_channels, depth=2),
                #nn.Linear是线性层，将task_token_channels的embedding映射到time_embed_dim的维度
    ...
    def forward(self, x, timesteps, y=None):
        ...
        hs = []
        #timestep_embedding是根据timesteps生成一个embedding，embedding的维度是model_channels * 4
        ...
        if self.task_tokens:
            #self.task_attnpool是根据y生成一个embedding，embedding的维度是task_token_channels
