...
def adaptive_groupnorm(num_channels, num_groups=32, **kwargs):
    g = min(num_groups, num_channels)
    while num_channels % g != 0 and g > 1:
        g -= 1
    return torch.nn.GroupNorm(num_groups=g, num_channels=num_channels, **kwargs)
...
def Normalize(in_channels):
    return adaptive_groupnorm(in_channels, num_groups=32, eps=1e-6, affine=True)
...
class CrossAttention(nn.Module):
    ...
    def __init__(self, query_dim, key_dim, value_dim, heads=8, dim_head=64, dropout=0.1, use_checkpoint=True):
        super().__init__()
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context, objs):
        print(f"x.shape: {x.shape}") #x.shape: torch.Size([2, 640, 7, 8, 8])
        b, c, h, w = x.shape #这里x的维度太大了，超过4了，需要检查一下
        x_in = x
        x = self.norm(x)
        x = self.proj_in(x)
