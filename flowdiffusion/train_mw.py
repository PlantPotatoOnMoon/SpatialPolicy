from goal_diffusion import GoalGaussianDiffusion_mw as GoalGaussianDiffusion, Trainer_mw as Trainer
from unet import UnetMW as Unet
from transformers import CLIPTextModel, CLIPTokenizer
from datasets import SequentialDatasetv2_mw as SequentialDatasetv2, parse_feedback_plan_mw as parse_feedback_plan
...
def main(args):
    ...
    if args.mode == 'inference':
        ...
        train_set = SequentialDatasetv2(
            ...
            path="/home//datasets/mw", 
            target_size=target_size,
            randomcrop=True
        )
        step = max(1, len(train_set)//valid_n)
        valid_inds = [i for i in range(0, len(train_set), step)][:valid_n]
    ...
    if args.mode == 'train':
        ...
        if args.batch_mode:
            ...
            inference_dl = DataLoader(inference_dataset, batch_size=8, shuffle=False, num_workers=0)
            ...
            for batch_idx, (val_xs_flat, val_x_conds, val_all_conditions) in enumerate(inference_dl):
                ...
                print(f"val_xs_flat.shape: {val_xs_flat.shape}")
                print(f"val_x_conds.shape: {val_x_conds.shape}")
            ...
            print(f"task_text: {text}")
            
            print(f"task_text: {all_conditions['task_text']}")
            print(f"action_ids: {all_conditions['plan_info']['action_ids'][0].cpu().numpy()}")
            print(f"dir_ids: {all_conditions['plan_info']['dir_ids'][0].cpu().numpy()}")
            print(f"dist_scalars: {all_conditions['plan_info']['dist_scalars'][0].cpu().numpy().flatten()}")
            print("=" * 50)
            ...
            video_tensor = pred_xs[0]  # Shape: (frames, channels, height, width)
            video_tensor = rearrange(video_tensor, 'f c h w -> f h w c')
            video_np = (video_tensor.numpy().clip(0, 1) * 255).astype('uint8')
