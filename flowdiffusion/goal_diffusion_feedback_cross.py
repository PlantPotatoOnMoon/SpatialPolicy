    ...
    def __init__(
        ...
        *,
        ...
        self.plan_proj = nn.Sequential(
            nn.Linear(192, 256), # 192 = 64+64+64
            nn.GELU(), 
            nn.Linear(256, text_embed_dim)
        )
        
        # --- B. Common Modules ---
        # 0: task, 1: plan (removed distance type)
...
# In goal_diffusion_ourdataset_task_feedback.py, inside GoalGaussianDiffusion class
    ...
    def _prepare_condition_embedding(self, all_conditions):
        ...
        typed_plan_embed = self.type_embedder(plan_embed, type_id=1) # Use type_id=1 for plan
    ...
    def model_predictions(self, x, t, x_cond, all_conditions, clip_x_start=False,rederive_pred_noise=False,  guidance_weight=0):
        ...
        if guidance_weight > 0.0:
            ...
            null_conditions = {
                'task_text': ["" for _ in range(x.shape[0])],  # 确保是列表，长度与 x 的 batch size 一致
                'plan_info': {
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        projection_keys = [key for key in model_state_dict.keys() if 'auto_proj' in key]
        if projection_keys:
    ...
    def save(self, milestone):
        ...
        def register_projection_layers(module):
            if hasattr(module, '_register_projection_parameters'):
                module._register_projection_parameters()
            for child in module.children():
                register_projection_layers(child)
        
        register_projection_layers(model)
        ...
        if not projection_keys:
            ...
            projection_keys = [key for key in full_state_dict.keys() if 'auto_proj' in key]
            if projection_keys:
                for key in projection_keys:
                    if key not in model_state_dict:
                        model_state_dict[key] = full_state_dict[key]
        ...
        save_path = str(self.results_folder / f'model-{milestone}{self.proc_id}.pt')
    ...
    def load(self, milestone):
        ...
        projection_keys = [key for key in data['model'].keys() if 'auto_proj' in key]

        total_params = sum(p.numel() for p in data['model'].values())

        model = self.accelerator.unwrap_model(self.model)
        
        def restore_projection_layers(module):
            if hasattr(module, '_restore_projection_layers'):
                module._restore_projection_layers()
            for child in module.children():
                restore_projection_layers(child)
        
        restore_projection_layers(model)
        ...
        if 'version' in data:
            print(f"[DEBUG] loading from version {data['version']}")
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            val_all_conditions = {
                                ...
                                'plan_info': {
                                    'action_ids': val_all_conditions['plan_info']['action_ids'][:num_samples_to_generate],
                                    'dir_ids': val_all_conditions['plan_info']['dir_ids'][:num_samples_to_generate],
                                    'dist_scalars': val_all_conditions['plan_info']['dist_scalars'][:num_samples_to_generate]
                                }
                            }
                        ...
                        print(f"\n[DEBUG] ====== 准备保存checkpoint (step {self.step}) ======")
                        
                        model = self.accelerator.unwrap_model(self.model)
                        model_state_dict = model.state_dict()
                        projection_keys = [key for key in model_state_dict.keys() if 'auto_proj' in key]
                        print(f"[DEBUG] 保存前模型中的投影层参数数量: {len(projection_keys)}")
                        if projection_keys:
                            print("[DEBUG] 保存前的投影层参数:")
                            for key in projection_keys:
                                param_shape = model_state_dict[key].shape
                                print(f"[DEBUG]   {key}: {param_shape}")
                        
                        ckpt_path = self.save(milestone)
                        
                        if ckpt_path and os.path.exists(ckpt_path):
                            checkpoint_data = torch.load(ckpt_path, map_location='cpu')
                            checkpoint_projection_keys = [key for key in checkpoint_data['model'].keys() if 'auto_proj' in key]
                            print(f"[DEBUG] 保存后checkpoint中的投影层参数数量: {len(checkpoint_projection_keys)}")
                            if checkpoint_projection_keys:
                                print("[DEBUG] 保存后checkpoint中的投影层参数:")
                                for key in checkpoint_projection_keys:
                                    param_shape = checkpoint_data['model'][key].shape
                                    print(f"[DEBUG]   {key}: {param_shape}")
                        
                        print(f"[DEBUG] ====== checkpoint保存完成 ======\n")
