...
NUM_THOR_ACTIONS = len(THOR_ACTION_TO_ID)
...
NUM_MW_ACTIONS = len(MW_ACTION_TO_ID)
...
for x in [-1, 0, 1]:
    for y in [-1, 0, 1]:
        for z in [-1, 0, 1]:
            vector = (x, y, z)
            MW_DIR_VECTOR_TO_ID[vector] = count
            MW_ID_TO_DIR_VECTOR[count] = vector
            count += 1
NUM_MW_DIRECTIONS = len(MW_DIR_VECTOR_TO_ID)
...
NUM_REAL_ACTIONS = len(REAL_ACTION_TO_ID)

REAL_DIR_TO_ID = {
    'up': 0, 'down': 1, 'left': 2, 'right': 3,
    'forward': 4, 'backward': 5, 'pad': 6
}
REAL_ID_TO_DIR = {v: k for k, v in REAL_DIR_TO_ID.items()}
NUM_REAL_DIRECTIONS = len(REAL_DIR_TO_ID)
...
# Base Gaussian Diffusion Class (shared logic)

class BaseGaussianDiffusion(nn.Module):
    """Base class with shared diffusion logic"""
    def __init__(
        self,
        model,
        tokenizer,
...
# Thor-specific Diffusion Model

class GoalGaussianDiffusion_thor(nn.Module):
    def __init__(
        self,
        model,
        tokenizer, #add
        ...
        *,
        ...
        NUM_ACTIONS = 5  # forward, left, right, done, pad
        
        # Feedback Plan Encoders for THOR 
        self.action_embedding = nn.Embedding(NUM_ACTIONS, 64)
        self.distance_scalar_mlp = nn.Sequential(
            nn.Linear(1, 64), 
            nn.GELU(), 
            nn.Linear(64, 64),
            nn.GELU(),
        ...
        self.plan_proj = nn.Sequential(
            nn.Linear(128, 256), # 128 = 64+64 (action + distance)
            nn.GELU(), 
        ...
        # Common Modules
        # 0: task, 1: plan
        self.type_embedder = TypeEmbedder(num_types=2, embedding_dim=text_embed_dim)
    ...
    def p_losses(self, x_start, t, x_cond, final_embed, noise=None):
    ...
    def _prepare_condition_embedding(self, all_conditions):
        if isinstance(all_conditions['task_text'], list):
            device = all_conditions['plan_info']['action_ids'].device
            task_text_list = all_conditions['task_text']
        else:
            device = all_conditions['plan_info']['action_ids'].device
            task_text_list = [all_conditions['task_text']]
            all_conditions = {
                'task_text': task_text_list,
                'plan_info': {
                    'action_ids': all_conditions['plan_info']['action_ids'].unsqueeze(0),
                    'dist_scalars': all_conditions['plan_info']['dist_scalars'].unsqueeze(0)
                }
            }
        ...
        with torch.no_grad():
            ...
            task_embed = self.text_encoder(
                input_ids=text_input_ids,
                attention_mask=text_attention_mask
            ).last_hidden_state


        action_ids = all_conditions['plan_info']['action_ids'].to(device)  # [batch_size, seq_len]
        dist_scalars = all_conditions['plan_info']['dist_scalars'].to(device)  # [batch_size, seq_len, 1]
        
        self.action_embedding = self.action_embedding.to(device)
        self.distance_scalar_mlp = self.distance_scalar_mlp.to(device)
        self.plan_proj = self.plan_proj.to(device)
        self.type_embedder = self.type_embedder.to(device)
        self.final_norm = self.final_norm.to(device)
        
        action_enc = self.action_embedding(action_ids)
        dist_enc = self.distance_scalar_mlp(dist_scalars)
        
        plan_feat = torch.cat([action_enc, dist_enc], dim=-1)
        plan_embed = self.plan_proj(plan_feat)
        

        typed_task_embed = self.type_embedder(task_embed, type_id=0)
        typed_plan_embed = self.type_embedder(plan_embed, type_id=1)
        

        combined_embed = torch.cat([typed_task_embed, typed_plan_embed], dim=1)
        final_embed = self.final_norm(combined_embed)
        
        final_embed = final_embed.to(device)
        print(f"Debug: _prepare_condition_embedding - device: {device}, final_embed device: {final_embed.device}, shape: {final_embed.shape}")
    ...
    def forward(self, img, img_cond, all_conditions):
        ...
        final_embed = final_embed * (torch.rand(final_embed.shape[0], 1, 1, device = final_embed.device) > self.cond_drop_chance).float()
        
        return self.p_losses(img, t, img_cond, final_embed)

# thor——trainer class

class Trainer_thor(object):
    def __init__(
        self,
        diffusion_model,
        tokenizer, 
    ...
    def train(self):
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process and self.wandb_project is not None:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            xs = []
                            task_embeds = []
                            for i, (x, x_cond, all_conditions) in enumerate(self.valid_dl):
                                xs.append(x)
                                x_conds.append(x_cond.to(device))
                                all_conditions['plan_info']['action_ids'] = all_conditions['plan_info']['action_ids'].to(device)
                                all_conditions['plan_info']['dist_scalars'] = all_conditions['plan_info']['dist_scalars'].to(device)
                                goal_embed = self.model._prepare_condition_embedding(all_conditions)
                                print(f"Debug: goal_embed device: {goal_embed.device}, shape: {goal_embed.shape}")
                                task_embeds.append(goal_embed)
                        ...
                        if self.step == self.save_and_sample_every:
                            ...
                            if self.accelerator.is_main_process and self.wandb_project is not None:
                                wandb.log({
                                    "ground_truth": wandb.Image(gt_img, caption="Ground Truth Reference")
                                }, step=self.step)
                        ...
                        utils.save_image(pred_img, str(self.results_folder / f'imgs/outputs/sample-{milestone}{self.proc_id}.png'), nrow=n_rows+2)

                        import tempfile
                        import imageio
                        if self.accelerator.is_main_process:
                            video_tensor = all_xs[:, :, :, :, :]  # shape: [B, T, C, H, W]
                            video_tensor = rearrange(video_tensor, 'b t c h w -> t b h w c')[:, 0] 
                            video_np = (video_tensor.numpy().clip(0, 1) * 255).astype('uint8')

                            with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as f:
                                video_path = f.name
                                imageio.mimsave(video_path, video_np, fps=2) 
                                if self.wandb_project is not None:
                                    wandb.log({
                                        f"eval/generated_video": wandb.Video(video_path, fps=2, format="mp4")
                                    }, step=self.step)
...
class GoalGaussianDiffusion_mw(BaseGaussianDiffusion):
    def __init__(self, model, tokenizer, text_encoder, image_size, **kwargs):
        super().__init__(model, tokenizer, text_encoder, image_size, **kwargs)
        
        text_embed_dim = self.text_encoder.config.hidden_size
        
        self.action_embedding = nn.Embedding(NUM_MW_ACTIONS, 64)
        self.direction_embedding = nn.Embedding(NUM_MW_DIRECTIONS, 64)
        self.distance_scalar_mlp = nn.Sequential(
            nn.Linear(1, 64),
            nn.GELU(),
            nn.Linear(64, 64)
    ...
    def _prepare_condition_embedding(self, all_conditions):
        ...
        plan_info = all_conditions['plan_info']
        action_ids = plan_info['action_ids'].to(device)
        dir_ids = plan_info['dir_ids'].to(device)
        dist_scalars = plan_info['dist_scalars'].to(device)
        
        action_enc = self.action_embedding(action_ids)
        dir_enc = self.direction_embedding(dir_ids)
        dist_enc = self.distance_scalar_mlp(dist_scalars)
        
        plan_feat = torch.cat([action_enc, dir_enc, dist_enc], dim=-1)
        plan_embed = self.plan_proj(plan_feat)
        typed_plan_embed = self.type_embedder(plan_embed, type_id=1)
    ...
    def model_predictions(self, x, t, x_cond, all_conditions, clip_x_start=False, rederive_pred_noise=False, guidance_weight=0):
        ...
        if guidance_weight > 0.0:
            null_conditions = {
                'task_text': ["" for _ in range(x.shape[0])],
                'plan_info': {
                    'action_ids': torch.zeros_like(all_conditions['plan_info']['action_ids']),
                    'dir_ids': torch.zeros_like(all_conditions['plan_info']['dir_ids']),
                    'dist_scalars': torch.zeros_like(all_conditions['plan_info']['dist_scalars'])
                }
            }
...
# Real-World-specific Diffusion Model

class GoalGaussianDiffusion_real(BaseGaussianDiffusion):
    def __init__(self, model, tokenizer, text_encoder, image_size, **kwargs):
        super().__init__(model, tokenizer, text_encoder, image_size, **kwargs)
        
        text_embed_dim = self.text_encoder.config.hidden_size
        
        self.dist_embedding = nn.Linear(1, 64)
        self.action_embedding = nn.Embedding(NUM_REAL_ACTIONS, 64)
        self.dir_embedding = nn.Embedding(NUM_REAL_DIRECTIONS, 64)
        self.plan_proj = nn.Linear(192, 512)
    ...
    def _prepare_condition_embedding(self, all_conditions):
        task_text_raw = all_conditions['task_text']
        task_text = [t[0] if isinstance(t, list) else t for t in task_text_raw]
        ...
        B_text, S_text, H_text = text_embed.shape

        plan_info = all_conditions['plan_info']
        action_embed = self.action_embedding(plan_info['action_ids'].to(self.device))
        dir_embed = self.dir_embedding(plan_info['dir_ids'].to(self.device))

        dist_scalars = plan_info['dist_scalars']
        if isinstance(dist_scalars, list):
            dist_scalars = torch.tensor(dist_scalars, device=self.device, dtype=torch.float32)
        if dist_scalars.ndim == 2:
            dist_scalars = dist_scalars.unsqueeze(-1)
        dist_embed = self.dist_embedding(dist_scalars.to(self.device))

        plan_embed = torch.cat([action_embed, dir_embed, dist_embed], dim=-1)
        B_plan, S_plan, H_plan = plan_embed.shape

        if B_text != B_plan:
            if B_text == 1:
                text_embed = text_embed.expand(B_plan, -1, -1).contiguous()
            elif B_plan == 1:
                plan_embed = plan_embed.expand(B_text, -1, -1).contiguous()
            else:
                raise ValueError(f"Batch mismatch: text {B_text}, plan {B_plan}")

        B = max(B_text, B_plan)

        if not hasattr(self, 'plan_proj'):
            self.plan_proj = nn.Linear(H_plan, H_text).to(self.device)
        plan_embed_proj = self.plan_proj(plan_embed)

        if S_text != S_plan:
            plan_embed_proj = torch.nn.functional.adaptive_avg_pool1d(
                plan_embed_proj.transpose(1, 2), S_text
            ).transpose(1, 2)

        final_embed = text_embed + plan_embed_proj
        return final_embed
...
class BaseTrainer:
    ...
    def encode_batch_text(self, batch_text):
        ...
        for text in batch_text:
            ...
            chunks = [input_ids[i:i + 75] for i in range(0, len(input_ids), 75)]
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            val_all_conditions = self._truncate_conditions(val_all_conditions, num_samples_to_generate)
                        ...
                        self._log_videos_to_wandb(pred_xs, milestone)
    ...
    def _truncate_conditions(self, all_conditions, num_samples):
        """Override in subclass if needed"""
        return all_conditions

    def _log_videos_to_wandb(self, pred_xs, milestone):
        """Override in subclass for specific video logging"""
        pass

# MetaWorld-specific Trainer

class Trainer_mw(BaseTrainer):
    def _truncate_conditions(self, all_conditions, num_samples):
        return {
            'task_text': all_conditions['task_text'][:num_samples],
            'plan_info': {
                'action_ids': all_conditions['plan_info']['action_ids'][:num_samples],
                'dir_ids': all_conditions['plan_info']['dir_ids'][:num_samples],
                'dist_scalars': all_conditions['plan_info']['dist_scalars'][:num_samples]
            }
        }
...
# Real-World-specific Trainer

class Trainer_real(BaseTrainer):
    def _truncate_conditions(self, all_conditions, num_samples):
        return {
            'task_text': all_conditions['task_text'][:num_samples],
            'plan_info': {
    ...
    def _log_videos_to_wandb(self, pred_xs, milestone):
        if self.accelerator.is_main_process:
            ...
            video_path = str(self.results_folder / f'sample-{milestone}{self.proc_id}.mp4')
            imageio.mimsave(video_path, video_np, fps=10)
            wandb.log({f"eval/generated_video_step_{self.step}": wandb.Video(video_path, fps=10, format="mp4")}, step=self.step)